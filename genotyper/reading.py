...
def constrain_chromosomes_by_assembly_report(chromosomes,assembly_report):
    ...
    if not assembly_report:
        logging.info("Not constraining by assembled chromosomes")
        return dict(zip(chromosomes,chromosomes))

    if assembly_report and not os.path.isfile(assembly_report):
        logging.error("No assembly report found at the provided location")
        raise ValueError(f"No assembly report at the {assembly_report}")

    with open(assembly_report) as _f:
        header = -1
        line = _f.readline()
    ...
    assrep = pd.read_csv(assembly_report,sep="\t",header=header)
...
def write_read_depth_file(pytor_files,output_file,assembly_report=None,compress="half",
    ...
    with hpy.File(output_file, "w") as output:
        ...
        for file in pytor_files:
            ...
            logging.info("For sample %s, binsize is %s, flag is %s, and global RD mean is %.2f",sample,binsize,flag,mean)
            ...
            logging.info("Written data for sample %s found at %s",sample,file)
            #time.sleep(5)
